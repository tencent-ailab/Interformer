...
# Clockwise dihedral2 from https://stackoverflow.com/questions/
# 20305272/dihedral-torsion-angle-from-four-points-in-cartesian-coordinates-in-python
...
def get_dihedral_vonMises(mol, conf, atom_idx, Z):
    ...
    for b1 in iAtom.GetBonds():
        ...
        for b2 in jAtom.GetBonds():
            ...
            print(k_0, i, j, l_0, k, l)
            print('->', f's_star:{s_star}, a_mat:{a_mat}')
            print('#' * 100)
            v = v + np.matmul(a_mat, s_star)  # [2,2] @ [2,1]
    print("v->", v)
    v = v / np.linalg.norm(v)
    v = v.reshape(-1)
    print("v_after_norm->", v)
    degree = np.degrees(np.arctan2(v[1], v[0]))
    print("Degree", degree, '\n', '=' * 100)
    return degree
...
if __name__ == '__main__':
    ...
    write_with_new_coords(optimized_mol, coords_pred_optimized, f'out/{pdb}_torsion_test.sdf')
    ########
    # write it out
    # writer = Chem.SDWriter('out/torsion_test.sdf')
